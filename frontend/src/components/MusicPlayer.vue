    ...
    <div class="player-content">
      ...
      <div class="info-controls">
        <div class="song-info" v-if="musicStore.currentSong">
          <n-text class="song-title" strong>{{
            musicStore.currentSong.title
          }}</n-text>
          <n-text class="artist-name" depth="3">{{
          ...
          >
            <template #icon>
              ...
              >
                ...
                <span
                  v-if="musicStore.playMode === 'single'"
                  style="
                    position: absolute;
                    font-size: 10px;
                    font-weight: bold;
        ...
        <n-tab-pane name="list" tab="当前播放">
          <n-list hoverable clickable>
            <n-list-item v-for="song in musicStore.playlist" :key="song.id">
              <template #prefix>
                <n-avatar
                  :src="song.cover || '/default-music.svg'"
                  size="small"
                  round
                />
              </template>
              <div class="song-list-item" @click="playSong(song)">
                ...
                <div class="song-list-action">
                  <n-icon
                    v-if="musicStore.currentSong?.id === song.id"
                    color="#8c7b75"
                    ><musical-notes
                  /></n-icon>
                  <n-button
