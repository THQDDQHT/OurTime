...
import com.neural.entity.Moment;
import com.neural.entity.Music;
import com.neural.entity.User;
import com.neural.repository.AlbumRepository;
import com.neural.repository.MomentRepository;
import com.neural.repository.MusicRepository;
...
public class DataInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (!albumsWithoutUser.isEmpty()) {
            log.info("发现 {} 个未绑定用户的相册，正在迁移 root 账号...", albumsWithoutUser.size());
        ...
        if (!momentsWithoutUser.isEmpty()) {
            log.info("发现 {} 个未绑定用户的瞬间，正在迁移 root 账号...", momentsWithoutUser.size());
        ...
        if (!musicWithoutUser.isEmpty()) {
            log.info("发现 {} 个未绑定用户的音乐，正在迁移 root 账号...", musicWithoutUser.size());
